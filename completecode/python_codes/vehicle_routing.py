...
appPath = "D:/SuveechiD/SuVeechi/DistanceCalculator/VRP/"


# Load the logger configuration
logging.config.fileConfig(fname=appPath + 'conf/logger.conf', disable_existing_loggers=False)
...
def create_data():
      ...
      for x in table1.find():
          ...
          logger.debug("Locations: %s", data['addresses'])
          print("Demands", data['demands'])
          logger.debug("Demands: %s", data['demands'])
          print("vehiclenumber:", vehicleNumber)
          logger.debug("vehiclenumber: %s", vehicleNumber)
          print("startpoint:", startpoint)
          logger.debug("startpoint: %s", startpoint)
          print("startpoint", data['starts'])
          logger.debug("startpoint: %s", data['starts'])
          print("endpoint", endpoint)
          logger.debug("endpoint: %s", endpoint)
          print("endpoint", data['ends'])
          logger.debug("endpoint: %s", data['ends'])
          print("vehiclecapacity", data['vehicle_capacities'])
          logger.debug("vehiclecapacity: %s", data['vehicle_capacities'])
...
def send_request(origin_addresses, dest_addresses, API_key):
  ...
  try:
      ...
      logger.debug("Request to get distance matrix %s",request)
      jsonResult = ur.urlopen(request).read()
      response = json.loads(jsonResult)
      print("response", response)
      logger.debug("Response from distance_matrix api %s", response)
      return response
  except Exception:
      print("Unable to call API")
      logger.warning("Unable to call API")
      exit()
...
def firstMain():
  ...
  data = create_data()
  distance_matrix = create_distance_matrix(data)    # Call create_distance_matrix function to create distance matrix
...
if __name__ == '__main__':
    try:
        ...
        if trip_table in mydatabase.list_collection_names() and vehicle_table in mydatabase.list_collection_names():
            print(mydatabase.list_collection_names())
            table1 = mydatabase['trips']  # Fetch data from trips table
            table2 = mydatabase['vehicles']  # Fetch data from vehicles table
            logger.debug("Fetched data from %s,%s table", trip_table,vehicle_table)
            firstMain()  # call firstMain function
        else:
            print("Unable to fetch data from collections")
    except Exception:
        print("Unable to call database function")
        logger.debug("Error reading data from mongodb table")

'''create_data_model to get optimum route'''
...
'''Calculate distance and time between the locations'''
def print_solution(data, manager, routing, assignment):
    ...
    for vehicle_id in range(len(data['vehicle_capacities'])):
        ...
        for i in name_list:
            ...
            '''Convert lat,long to address of vehicle routes'''
            try:
                ...
                if response_text != '' and len(response_text) > 100:
                    ...
                    logger.debug("Route address %s", parsedJson["display_name"])
            except Exception:
                print("Unable to call API")
        print("routeaddress", routeaddress)
        logger.debug("routeaddress %s",routeaddress)
        print("new_list", seplatlong)

        print("name_list", name_list)


        '''Calculate time for each locations of the route'''
        ...
        logger.debug(("Tota_Time %s",ttime))
        plan_output = '{}'.format(route_distance)
        plan_output = int(plan_output) / (1000)
        Total_Distance = str(plan_output) + 'km'
        logger.debug("Total Distance %s", Total_Distance)
        ...
        collection = mydatabase.routes.insert_one(
            {"vehicleNo": data['vehicleNumber'][vehicle_id], "routeforvehicle": seplatlong, "routeaddress":routeaddress,
             "Distanceforeachlocation": distance_list, "Totaldistanceforroute": Total_Distance,
        ...
        logger.debug("Resultant route inserted successfully to mongodb %s",collection)
...
''' Call second_main function to get route for vehicle'''
if __name__ == '__main__':
    second_main()